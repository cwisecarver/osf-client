...
import {User, Node, Registration, Collection, Comment, Log, File} from './models.es6.js';
...
class Client {
  ...
  registrations(identifier) {
    this._identifier = identifier;
    return this.clientFactory('registrations/', Registration, ['get']);
  }

  collections(identifier) {
    this._identifier = identifier;
    return this.clientFactory('collections/', Collection, ['create', 'get', 'update', 'delete']);
  }

  comments(identifier) {
    this._identifier = identifier;
    return this.clientFactory('comments/', Comment, ['create', 'get', 'update'], false);
  }

  logs(identifier) {
    this._identifier = identifier;
    return this.clientFactory('logs/', Log, ['get'], false);
  }
